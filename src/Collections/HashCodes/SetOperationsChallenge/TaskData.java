...
import Collections.HashCodes.SetOperationsChallenge.Task.Priorities;
import Collections.HashCodes.SetOperationsChallenge.Task.Status;
...
public class TaskData {
  ...
  */
  ...
  public Set<Task> getTasks(String owner) {
    ...
    String user  = "ana,bob,carol".contains(owner.toLowerCase()) ? owner : null;
    ...
    for (String taskData : selectedList.split("\n")) {
      ...
      Status status = (data.length <= 3) ? Status.IN_PROGRESS : Status.valueOf(data[3].toUpperCase().replace(" ", "_"));

      Priorities priority = Priorities.valueOf(data[2].toUpperCase());
